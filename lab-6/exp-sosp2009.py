...
def main():
    ...
    operation = {
        "detect": [
            SequentialAnomalyDetection,
            SyntacticDistanceAndStrategyAnalysis,
            RankBasedAreaUnderCurveAnalysis,
        ],
        "seqsa": [
            SyntacticDistanceAndStrategyAnalysis,
        ],
        ...
        "parse": [
            ParseFromOriginalData,
        ],
        "lkass": [
            ListKeyAndSequenceStatistics,
        ],
        "statkey": [
            PrintKeyAndStatistics,
        ],
        "sdplt": [
            PlotSyntacticDistance,
        ],
        "report": [
            ReportAnomalies,
        ],
    ...
    parser = argparse.ArgumentParser(
        description="Experiment based on SOSP 2009 dataset"
    )
    parser.add_argument(
        "operation", help="Specify an operation (" + ", ".join(operation.keys()) + ")"
    )
    # Anomaly Detection
    parser.add_argument(
        "-l",
        "--label",
        help="Specify a label file (e.g., mylabel.sample_1024.txt",
        default="label.txt",
    )
    parser.add_argument(
        "-i",
        "--input",
        help="Specify a input file or directory that contains data (e.g., mydata/)",
        default="data",
    )
    parser.add_argument(
        "-o",
    ...
    parser.add_argument(
        "-m", "--model", help="Specify a deep learning model", default="dablog"
    )
    ...
    parser.add_argument(
        "--logkeys",
        help="Logkeys Scenarios (0=32, 1=78, 2=104, 3=1129)",
        type=int,
        default=2,
    )
    parser.add_argument(
        "--train-normal",
        help="Number of normal blocks in training set",
        type=int,
        default=200000,
    )
    parser.add_argument(
        "--train-abnormal",
        help="Number of abnormal blocks in training set",
        type=int,
        default=0,
    )
    parser.add_argument(
        "--test-normal",
        help="Number of normal blocks in testing set",
        type=int,
        default=200000,
    )
    parser.add_argument(
        "--test-abnormal",
        help="Number of abnormal blocks in testing set",
        type=int,
        default=16838,
    )
    parser.add_argument(
        "--save-model", help="Specify a filepath to which to save the model"
    )
    parser.add_argument(
        "--load-model", help="Specify a filepath from which to load the model"
    )
    parser.add_argument(
        "--sdasa-strategy",
        help="Specify a strategy [rank|dist|complete|all]",
        default="all",
    )
    parser.add_argument(
        "--sdasa-distance", help="Specify a distance threshold", type=float, default=0.0
    )
    parser.add_argument(
        "--use-gpu",
        help="Specify GPU usage (-1: disable, default: all gpus)",
        type=int,
        nargs="+",
    )
    parser.add_argument(
        "--use-mimick", help="Toggle Mimick Embedding", action="store_true"
    )
    parser.add_argument(
        "--stats-step", help="Step for stats (default=0.01)", type=float, default=0.01
    )
    parser.add_argument(
        "--check-adjacent", help="Check adjacent sequences", type=int, default=0
    )
    # Parser
    ...
    for line in open(filepath, "r"):
        ...
        if args.logkeys != 0:
            ...
            if "src_ip" in evt and "dst_ip" in evt:
                ...
                elif ".".join(evt["src_ip"].split(".")[:precision]) == ".".join(
                    evt["dst_ip"].split(".")[:precision]
                ):
                    task += ", traffic=subnet"
                else:
...
def SequentialAnomalyDetection(args):
    ...
    if args.model in ["deeplog"]:
        trainset = DeeplogSequence(
            DeeplogSequence.Config(seqlen=args.seqlen, verbose=True)
        )
        Model = Deeplog
    if args.model in ["autoencoder", "dablog"]:
        trainset = DablogSequence(
            DablogSequence.Config(seqlen=args.seqlen, verbose=True)
        )
        Model = Dablog

    Model.SetupGPU(args.use_gpu)
    # read labels
    normals, abnormals, count, bar = (
        [],
        [],
        0,
        util.ProgressBar("Read Anomaly Labels", 575139),
    )
    ...
    args.train_normal, args.train_abnormal = min(args.train_normal, len(normals)), min(
        args.train_abnormal, len(abnormals)
    )
    if args.train_normal > 0:
        ...
        for i in range(0, args.train_normal):
            ...
            try:
                trainset.append(
                    readSequence(os.path.join(args.input, normals[i] + ".log"), args),
    ...
    if args.train_abnormal > 0:
        ...
        for i in range(0, args.train_abnormal):
            ...
            try:
                trainset.append(
                    readSequence(os.path.join(args.input, abnormals[i] + ".log"), args),
    ...
    config = Model.Config(
        epochs=args.epochs,
        use_mimick_embedding=args.use_mimick,
        filepath=args.output,
        rank_threshold=args.stats_step,
        distance_threshold=args.stats_step,
        verbose=True,
    )
    model = Model(config)
    try:
    ...
    del trainset
    gc.collect()
    args.test_normal, args.test_abnormal = min(args.test_normal, len(normals)), min(
        args.test_abnormal, len(abnormals)
    )
        ...
        json.dumps(
            {
                ...
                "seqlen": args.seqlen,
                "epochs": args.epochs,
                "logkeys": args.logkeys,
                "train-normal": args.train_normal,
                "train-abnormal": args.train_abnormal,
                "test-normal": args.test_normal,
                "test-abnormal": args.test_abnormal,
            }
        )
        + "\n"
    )
...
def ReportAnomalies(args):
    ...
    for progress in range(0, metadata["misses"]):
        ...
        if metadata["model"] in ["autoencoder", "dablog"]:
            ...
            I["dist"] = numpy.linalg.norm(
                [dij[X[i]][Y[0 - i - 1]] for i in range(0, len(X))]
            )
    ...
    if True:
        ...
        if metadata["model"] in ["deeplog"]:
            ...
            for key in keys:
                ...
                data.append(vector)
        hypertools.plot(
            numpy.array(data), ".", labels=labels, hue=hue, explore=True, legend=True
        )
...
def RankBasedAreaUnderCurveAnalysis(args):
    ...
    with open(os.path.join(args.input, "subjectranks"), "w") as fout:
        for subject in subjectranks:
            fout.write(json.dumps(subjectranks[subject]) + "\n")
    # sort subject ranks
    ...
    ranks = sorted(
        list(ranks.items()), reverse=True, key=lambda obj: obj[0]
    )  # sort by rank
    # write absolute FNs
    ...
    with open(os.path.join(args.input, "rank.metric"), "w") as fout:
        steps = [
            i
            for i in numpy.arange(args.stats_step, 1 + args.stats_step, args.stats_step)
        ]
...
def SyntacticDistanceAndStrategyAnalysis(args):
    ...
    normals, abnormals = metadata["test-normal"], metadata["test-abnormal"]
    steps = [
        i for i in numpy.arange(args.stats_step, 1 + args.stats_step, args.stats_step)
    ]
    ...
    bar = util.ProgressBar(
        "Syntatic Distance and Strategy Analysis (" + strategy + ")", metadata["misses"]
    )
    try:
        fin = open(os.path.join(args.input, "predict"), "r")
    except:
    ...
    for progress in range(0, metadata["misses"]):
        ...
        for miss in misses:
            ...
            # strategy : distance or complete
                ...
                spred = sorted(
                    [i for i in range(0, len(P))],
                    reverse=True,
                    key=lambda i: P[i] * 1000000 - dij[x][i],
                )
    ...
    stats = modelUtil.Statistics(
        steps,
        metadata["seqlen"] + 1,
        metadata["test-normal"],
        metadata["test-abnormal"],
    )
        ...
        for i in range(0, len(positives[subject])):
            ...
            for obj in positives[subject][i]:
                ...
                if obj["len"] < args.seqlen:
                    ...
                    [idx + adj not in seqs for adj in range(0, args.check_adjacent + 1)]
                ):
                    stats.add(i, obj)
    fin.close()
    bar.finish()
...
def ListKeyAndSequenceStatistics(args):
    ...
    sort = sorted(
        list(evtset.keys()), reverse=True, key=lambda k: evtset[k]["occurrence"]
    )
    with open(
        ".".join([args.output, str(args.logkeys), str(len(sort)), "keys"]), "w"
    ) as fout:
        for key in sort:
            obj = evtset[key]
                ...
                json.dumps(
                    {
                        "key": key,
                        "occurrence": occ,
                        "percentage": occ / count,
                        "normals": obj["Normal"],
                        "abnormals": obj["Abnormal"],
                    }
                )
                + "\n"
            )
    ...
    with open(
        ".".join([args.output, str(args.logkeys), str(args.seqlen), "seqs"]), "w"
    ) as fout:
        for seqstr in seqset:
            ...
            obj = {
                "sequence": json.loads(seqstr),
                "occurrence": occurrence,
                "percentage": occurrence / count,
                "normals": normals,
                "abnormals": abnormals,
            }
            fout.write(json.dumps(obj) + "\n")
...
def PlotSyntacticDistance(args):
    ...
    if args.output == "embed":
        ...
        hypertools.plot(
            numpy.array(data), ".", labels=labels, n_clusters=10, explore=True
        )
        ...
        for hue, dataset in [
            ...
            for i in range(0, min(4096, len(dataset))):
                ...
                labels.append(hue)
        labels[0] = (
            None  # workaround of hypertools keeps showing numbers instead of texts in its legend
        )
...
def ParseFromOriginalData(args):
    def parseEvent(splits):
        ...
        if "ERROR dfs.DataNode$DataXceiver:" == " ".join(splits[:2]):
            event["task"] = (
                "ERROR dfs.DataNode$DataXceiver: DST:DataXceiver: EXCEPTION: block BLK "
                + " ".join(splits[6:])
            )
            event["block_id"] = splits[5]
            event["exception"] = splits[3]
            event["dst_ip"], event["dst_port"], _, _ = (
                splits[2].replace("/", "").split(":")
            )
        elif "INFO dfs.DataBlockScanner: Verification succeeded for" == " ".join(
            splits[:5]
        ):
            event["task"] = "INFO dfs.DataBlockScanner: Verification succeeded for BLK"
            event["block_id"] = splits[-1]
        elif (
            "INFO dfs.DataNode$BlockReceiver: Changing block file offset of block"
            == " ".join(splits[:8])
        ):
            event["task"] = (
                "INFO dfs.DataNode$BlockReceiver: Changing block file offset of block BLK from PARAM1 to PARAM2 meta file offset to PARAM3"
            )
            event["block_id"] = splits[8]
            event["param"] = [splits[10], splits[12], splits[-1]]
        elif (
            "INFO dfs.DataNode$BlockReceiver: Exception in receiveBlock for block"
            == " ".join(splits[:7])
        ):
            event["task"] = (
                "INFO dfs.DataNode$BlockReceiver: Exception in receiveBlock for block BLK EXCEPTION"
            )
            event["block_id"] = splits[7]
            event["exception"] = " ".join(splits[8:])
        elif "INFO dfs.DataNode$BlockReceiver:" == " ".join(
            splits[:2]
        ) and "writing block" == " ".join(splits[3:5]):
            event["task"] = (
                "INFO dfs.DataNode$BlockReceiver: SRC:Exception writing block BLK to mirror DST"
            )
            event["block_id"] = splits[5]
            event["src_ip"], event["src_port"], _ = (
                splits[2].replace("/", "").split(":")
            )
            event["dst_ip"], event["dst_port"] = splits[-1].replace("/", "").split(":")
        elif (
            "INFO dfs.DataNode$BlockReceiver: Receiving empty packet for block"
            == " ".join(splits[:7])
        ):
            event["task"] = (
            ...
            event["task"] = (
                "INFO dfs.DataNode$DataTransfer: SRC:Transmitted block BLK to DST"
            )
            ...
            "INFO dfs.DataNode$DataXceiver:" == " ".join(splits[:2])
            and "Served" == splits[3]
        ):
            ...
            event["task"] = (
                "INFO dfs.DataNode$DataXceiver: Received block BLK src: SRC dest: DST of size SIZE"
            )
            event["block_id"] = splits[4]
            event["src_ip"], event["src_port"] = (
                splits[4 + 2].replace("/", "").split(":")
            )
            event["dst_ip"], event["dst_port"] = (
                splits[4 + 4].replace("/", "").split(":")
            )
            ...
            "INFO dfs.DataNode$DataXceiver: writeBlock" == " ".join(splits[:3])
            and "exception" in splits
        ):
            event["task"] = (
                "INFO dfs.DataNode$DataXceiver: writeBlock BLK received exception EXCEPTION"
            )
            event["block_id"] = splits[3]
            event["exception"] = " ".join(splits[6:])
        elif "INFO dfs.DataNode:" == " ".join(
            splits[:2]
        ) and "Starting thread to transfer block" == " ".join(splits[3:8]):
            event["task"] = (
                "INFO dfs.DataNode: SRC Starting thread to transfer block BLK to DST(s)"
            )
            event["block_id"] = splits[8]
            event["dst_list"] = splits[10:]
        elif (
            "INFO dfs.DataNode$PacketResponder: PacketResponder" == " ".join(splits[:3])
            and "Exception" in splits
        ):
            event["task"] = (
                "INFO dfs.DataNode$PacketResponder: PacketResponder BLK CODE Exception EXCEPTION"
            )
            event["block_id"] = splits[3]
            event["code"] = splits[4]
            event["exception"] = " ".join(splits[6:])
            ...
            "INFO dfs.DataNode$PacketResponder: PacketResponder" == " ".join(splits[:3])
            and "Exception" not in splits
        ):
            event["task"] = (
                "INFO dfs.DataNode$PacketResponder: PacketResponder CODE for block BLK "
                + splits[-1]
            )
            event["block_id"] = splits[6]
            event["code"] = splits[3]
        elif "INFO dfs.DataNode$PacketResponder: Received block" == " ".join(
            splits[:4]
        ):
            event["task"] = (
                "INFO dfs.DataNode$PacketResponder: Received block BLK of size SIZE from IP"
            ...
            event["block_id"] = splits[-1]
        elif "INFO dfs.FSNamesystem: BLOCK* ask" == " ".join(
            splits[:4]
        ) and "to delete" == " ".join(splits[5:7]):
            ...
            splits[:4]
        ) and "to replicate" == " ".join(splits[5:7]):
            event["task"] = (
                "INFO dfs.FSNamesystem: BLOCK* ask DST to replicate BLK to datanode(s) SRC"
            )
            event["block_id"] = splits[7]
            event["src_ip"], event["src_port"] = splits[-1].replace("/", "").split(":")
            event["dst_ip"], event["dst_port"] = splits[4].replace("/", "").split(":")
        elif (
            "INFO dfs.FSNamesystem: BLOCK* NameSystem.addStoredBlock: addStoredBlock request received for"
            == " ".join(splits[:8])
        ):
            event["task"] = (
                "INFO dfs.FSNamesystem: BLOCK* NameSystem.addStoredBlock: addStoredBlock request received but does not belong to any file"
            )
            event["block_id"] = splits[8]
            event["dst_ip"], event["dst_port"] = (
                splits[8 + 2].replace("/", "").split(":")
            )
        ...
        elif "INFO dfs.FSNamesystem: BLOCK* NameSystem.allocateBlock:" == " ".join(
            splits[:4]
        ):
            event["task"] = (
                "INFO dfs.FSNamesystem: BLOCK* NameSystem.allocateBlock FILEPATH BLK"
            )
            event["block_id"] = splits[-1]
            ...
            event["block_id"] = splits[5]
        elif "WARN dfs.DataBlockScanner: Adding an already existing block" == " ".join(
            splits[:-1]
        ):
            event["task"] = (
                "WARN dfs.DataBlockScanner: Adding an already existing block BLK"
            )
            event["block_id"] = splits[-1]
        elif "WARN dfs.DataNode$DataTransfer:" == " ".join(splits[:2]):
            event["task"] = (
                "WARN dfs.DataNode$DataTransfer: SRC:Failed to transfer BLK to DST got EXCEPTION"
            )
            event["block_id"] = splits[5]
            event["src_ip"], event["src_port"], _ = (
                splits[2].replace("/", "").split(":")
            ...
            event["task"] = (
                "WARN dfs.DataNode$DataXceiver: SRC:Got exception while serving BLK to DST"
            )
            event["block_id"] = splits[6]
            event["src_ip"], event["src_port"], _ = (
                splits[2].replace("/", "").split(":")
        ...
        elif "WARN dfs.FSDataset: Unexpected error trying to delete block" == " ".join(
            splits[:8]
        ):
            event["task"] = (
            ...
            "WARN dfs.FSNamesystem: BLOCK* NameSystem.addStoredBlock: Redundant addStoredBlock request received for"
            == " ".join(splits[:9])
        ):
            event["task"] = (
                "WARN dfs.FSNamesystem: BLOCK* NameSystem.addStoredBlock: Redundant addStoredBlock request received for BLK on DST size SIZE"
            )
            event["block_id"] = splits[9]
            event["dst_ip"], event["dst_port"] = (
                splits[9 + 2].replace("/", "").split(":")
            )
            event["size"] = splits[9 + 4]
        elif (
            "WARN dfs.PendingReplicationBlocks$PendingReplicationMonitor: PendingReplicationMonitor timed out block"
            == " ".join(splits[:-1])
        ):
            event["task"] = (
