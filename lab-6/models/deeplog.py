    ...
    def getItemDeeplog(self, idx):
        ...
        for i in range(begin, end):
            ...
            for j in range(1, len(sequence)):
                xl = [
                    self.codebook.enc(event[0])
                    for event in sequence[max(0, j - self.config.seqlen) : j]
                ]
                xf = [
                    event[1:] for event in sequence[max(0, j - self.config.seqlen) : j]
                ]
        ...
        YL = keras.utils.to_categorical(
            YL, num_classes=len(self.codebook) - self.unknowns
        )
    ...
    def getItemWord2Vec(self, idx, epochs=1):
        ...
        for event in self.types:
            if event not in self.codebook.encode:
                ...
                self.unknowns += 1
                print(
                    "    \033[91mAdd Unknown Event to Codebook:\033[0m",
                    self.codebook.enc(event),
                    event,
                )
        # build vectors
        vectors, maxlen = [], 0
...
class Deeplog(object):
    ...
    def pretrain(self, trainset, mode="deeplog"):
        ...
        label_embed = keras.layers.Embedding(
            n_labels, self.config.hidden_unit, mask_zero=True, name="Label_Embed"
        )
        ...
        if n_floats == 0:
            ...
            model = keras.models.Model(
                inputs=[label_input, float_input], outputs=nn(merge)
            )
        model.compile(
        ...
        for i in range(0, len(X) // len(embedding)):
            Y.extend(embedding)
        Y = numpy.array(Y)
        self.mimick = mimick.Word2VecMimickEmbedding(
            X.shape[2],
            Y.shape[1],
    ...
    def fit(self, trainset):
        if self.model is None:
            ...
            if self.mimick is None:
                ...
                self.label_embed = keras.layers.Embedding(
                    self.n_labels,
                    self.config.hidden_unit,
                    mask_zero=True,
                    name="Label_Embed",
                )
                self.label_dense = self.label_embed(self.label_input)
            else:
                self.label_input = keras.layers.Input(
                    shape=(None, self.config.hidden_unit), name="Label_Input"
                )
                self.label_dense = keras.layers.Dense(self.config.hidden_unit)(
                    self.label_input
                )
                # self.label_dense = keras.layers.Dense (self.config.hidden_unit) (self.label_dense)
            # input float sequence
            if self.n_floats > 0:
                self.float_input = keras.layers.Input(
                    shape=(None, self.n_floats), name="Float_Input"
                )
                self.float_dense = keras.layers.Dense(
                    self.n_floats, name="Float_Dense"
                )(self.float_input)
                if self.mimick is None:
                    self.merge = keras.layers.concatenate(
                        [self.label_dense, self.float_dense], axis=2
                    )
                else:
                    self.merge = keras.layers.concatenate(
                        [self.label_input, self.float_dense], axis=2
                    )
            # neural network layer
            ...
            self.nn.add(
                keras.layers.LSTM(
                    self.nn_size, name="LSTM_" + str(self.config.hidden_layer)
                )
            )
            self.nn.add(
                keras.layers.Dense(self.n_labels, activation="softmax", name="Softmax")
            )
            # model
            if self.n_floats == 0:
                if self.mimick is None:
                    self.model = keras.models.Model(
                        inputs=self.label_input, outputs=self.nn(self.label_dense)
                    )
                else:
                    self.model = keras.models.Model(
                        inputs=self.label_input, outputs=self.nn(self.label_input)
                    )
            else:
                self.model = keras.models.Model(
                    inputs=[self.label_input, self.float_input],
                    outputs=self.nn(self.merge),
                )
            self.model.compile(
                loss="categorical_crossentropy", optimizer="adam", metrics=["acc"]
            )
    ...
    def predict(self, testset):
        ...
        for i in range(0, len(Y)):
            ...
            #    prob_threshold = sortProbs[
            #        min(int(self.config.rank_threshold * len(Y[i])) + 1, len(Y[i]) - 1)
            #    ]
            #    dists = [
            #        dij[label][j] if P[i][j] > prob_threshold else 1.0
            ...
            if append_to_result:  # append prediction-miss
                fout.write(
                    json.dumps(
                        {
                            "X": X[i].tolist() + [int(numpy.argmax(Y[i]))],
                            "P": P[i].tolist(),
                            "I": I[i],
                        }
                    )
